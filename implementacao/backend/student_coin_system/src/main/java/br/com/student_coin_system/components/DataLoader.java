...
public class DataLoader implements ApplicationListener<ContextRefreshedEvent> {
    ...
    private void criarDepartamentos(Instituicao instituicao) {
        for (int i = 0; i < 3; i++) { // Garante 3 departamentos
            ...
            // Verifica se o departamento já está associado para evitar duplicidade
            if (!instituicao.getDepartamentos().contains(departamento)) {
                instituicao.getDepartamentos().add(departamento); // Adiciona o departamento à instituição
            }
        }
        instituicaoRepository.save(instituicao); // Atualiza a instituição com seus departamentos
    }
    ...
    private void criarCursos(Instituicao instituicao) {
        for (int i = 0; i < 3; i++) { // Garante 3 cursos
            ...
            curso.setDepartamentos(new ArrayList<>()); // Define departamentos vazios inicialmente
    ...
    private void criarVantagens(Instituicao instituicao) {
        ...
        for (int i = 0; i < 3; i++) { // Garante 3 vantagens
            ...
            vantagemRepository.save(vantagem);
            instituicao.getVantagens().add(vantagem);  // Adiciona a vantagem à instituição
        }
        instituicaoRepository.save(instituicao);  // Atualiza a instituição com suas vantagens
    }
}